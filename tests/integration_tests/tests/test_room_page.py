...
class TestHotelPage():
    ...
    def test_room_elements_present(self):
        ...
        hotels_page.get_hotels[random.randint(0, len(hotels_page.get_hotels)-1)].click()
        hotel_page =self.hotel_page
        rooms=hotel_page.get_view_more
        #for now without api i just made a click on first room, when API will be ready - i can check that correct room is opened
        rooms[0].click()
        time.sleep(2)
        room_page= self.room_page
        Assert.true(room_page.is_room_visible, "Room description is missing")
        Assert.true(room_page.are_room_prices_visible, "Room prices are  missing")
        Assert.true(room_page.is_room_amentities_visible, "Room amentities is  missing")
    ...
    def test_opened_room_rate_all_area(self):
        ...
        open_rate= room_page.get_room_rates_names[2].text
        room_page.get_room_rates_area[2].click()
        time.sleep(1)
        Assert.true(room_page.opened_room_rate_is_visible, "Room rate didn't open")
        room= room_page.get_opened_room_name
        #print room.text
    ...
    def test_opened_room_rate_name_click(self):
        ...
        #for now without api i just made a click on second room rate, when API will be ready - i can check that correct rate is opened
        open_rate= room_page.get_room_rates_names[2].text
        room_page.get_room_rates_names[2].click()
        time.sleep(1)
        Assert.true(room_page.opened_room_rate_is_visible, "Room rate didn't open")
        room= room_page.get_opened_room_name
        #print room.text
